...
**********************************************************************;
DATA Example_54;
  LENGTH Stratum 3 Nh UnitCost Revenue Emplyees Revnu_SD Emply_SD 
         RCredit OffShore 8;
  ...
  INPUT Stratum Nh UnitCost Revenue Emplyees Revnu_SD Emply_SD 
        RCredit OffShore;
  CARDS;
 ...
 *_____ LOAD PARAMETERS _____*;
   ...
   NUMBER Nh{1..5};
   READ DATA Example_54 INTO [_n_] Nh;
   PRINT Nh;
                                                        *Per Unit Cost;
   NUMBER UnitCost{1..5};
  ...
  *_____ CONSTRAINTS _____*;
   ...
   CON SampSize{i in 1..5}: 100 <= NSamp[i] <= Nh[i];
   ...
   CON RelVar1: 
       (SUM{i in 1..5} Nh[i] * (Nh[i]/NSamp[i] - 1) * Emply_SD[i]^2) 
        / ((SUM{i in 1..5} Nh[i] * Emplyees[i])^2)
       <= (0.05^2);
   PRINT ((SUM{i in 1..5} Nh[i] * (Nh[i]/NSamp[i] - 1) * Emply_SD[i]^2) 
          / ((SUM{i in 1..5} Nh[i] * Emplyees[i])^2));
  ...
  *_____ OBJECTIVE FUNCTION _____*;
   ...
   MIN f = (SUM{i in 1..5} Nh[i] * (Nh[i]/NSamp[i] - 1) * Revnu_SD[i]^2) 
  ...
  *_____ SOLUTION _____*;
   ...
   SOLVE;
     ...
     PRINT (SQRT((SUM{i in 1..5} Nh[i] * (Nh[i]/NSamp[i] - 1) * Revnu_SD[i]^2) 
                 / ((SUM{i in 1..5} Nh[i] * Revenue[i])^2)));
     PRINT (SQRT((SUM{i in 1..5} Nh[i] * (Nh[i]/NSamp[i] - 1) * Emply_SD[i]^2) 
                 / ((SUM{i in 1..5} Nh[i] * Emplyees[i])^2)));
     PRINT (SQRT((SUM{i in 1..5} Nh[i] * (Nh[i]/NSamp[i] - 1) * RCrdt_SD[i]^2) 
                 / ((SUM{i in 1..5} Nh[i] * RCredit[i])^2)));
     PRINT (SQRT((SUM{i in 1..5} Nh[i] * (Nh[i]/NSamp[i] - 1) * OffSh_SD[i]^2) 
                 / ((SUM{i in 1..5} Nh[i] * OffShore[i])^2)));
