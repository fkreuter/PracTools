#***********************************************************************************************************
# FILE:     C:\Projects\Practical Tools Book\Book Chapters\15 Calibration\Examples\poststrat.undercoverage.R
...
cv(svyby(~as.factor(medicaid), ~hisp.r, nhis.dsgn, svymean, na.rm=TRUE))
        ...
        # estimates for age groups
svyby(~as.factor(medicaid), ~age.grp, ps.dsgn, svytotal, na.rm=TRUE)
svyby(~as.factor(medicaid), ~age.grp, ps.dsgn, svymean, na.rm=TRUE)

svyby(~as.factor(medicaid), ~age.grp, nhis.dsgn, svytotal, na.rm=TRUE)
svyby(~as.factor(medicaid), ~age.grp, nhis.dsgn, svymean, na.rm=TRUE)
